...
import static fr.aureliejosephine.mareu.utils.CalendarUtil.afterOrSameDate;
import static fr.aureliejosephine.mareu.utils.CalendarUtil.beforeOrSameDate;
import static fr.aureliejosephine.mareu.utils.CalendarUtil.sameDate;
...
public class MeetingUtil {
    ...
    public static List<Meeting> getMeetingsBeforeOrSameDate(Calendar date, List<Meeting> meetings) {
        ...
        for (Meeting m: meetings)
            if (beforeOrSameDate(m.getDate(), date))
                tmp.add(m);
    ...
    public static List<Meeting> getMeetingsAfterOrSameDate(Calendar date, List<Meeting> meetings) {
        ...
        for (Meeting m: meetings)
            if (afterOrSameDate(m.getDate(), date))
                tmp.add(m);
    ...
    public static List<Meeting> getMeetingsMatchRoomName(String roomName, List<Meeting> meetings) {
        ...
        for (Meeting m: meetings)
            if (m.getRoom().trim().equals(roomName.trim()))
                tmp.add(m);
